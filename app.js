...
function renderCaixa() {
  ...
  page.innerHTML = `
    ...
    <div class="caixa-layout">
      ...
      <div class="card">
        ...
        <div class="caixa-cat-tabs" id="caixa-cat-tabs">
          ${cats.map(c => `<button class="caixa-cat-btn ${_caixaCategoria === c ? 'active' : ''}" onclick="selecionarCategoriaCaixa('${c}')">${c}</button>`).join('')}
        </div>
  ...
  renderCaixaProdutos(_caixaCategoria === 'Todos' ? _produtos : _produtos.filter(p => p.categoria === _caixaCategoria));
  renderCaixaResumo();
}
...
function selecionarCategoriaCaixa(cat) {
  _caixaCategoria = cat;
  document.querySelectorAll('.caixa-cat-btn').forEach(b => b.classList.toggle('active', b.textContent === cat));
  filtrarCaixaProdutos();
}

function filtrarCaixaProdutos() {
  const q = document.getElementById('caixa-search')?.value.toLowerCase() || '';
  let lista = _caixaCategoria === 'Todos' ? _produtos : _produtos.filter(p => p.categoria === _caixaCategoria);
  if (q) lista = lista.filter(p => p.nome.toLowerCase().includes(q));
  renderCaixaProdutos(lista);
}
